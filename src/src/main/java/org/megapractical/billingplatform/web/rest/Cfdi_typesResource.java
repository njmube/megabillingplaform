...
public class Cfdi_typesResource {
    ...
    @RequestMapping(value = "/cfdi-types",
        ...
        params = {"filtername"})
    @Timed
    public ResponseEntity<List<Cfdi_types>> getAllCfdi_types(@RequestParam(value = "filtername") String filtername,
                                                             Pageable pageable)
        ...
        if(filtername.compareTo(" ")==0 || filtername.isEmpty()) {
            ...
            Page<Cfdi_types> page = cfdi_typesService.findAllByName(filtername,pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/cfdi-types");
            return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
        }
    }
