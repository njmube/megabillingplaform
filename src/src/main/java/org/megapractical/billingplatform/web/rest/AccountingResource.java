...
public class AccountingResource {
    ...
    public ResponseEntity<Accounting> createAccounting(@Valid @RequestBody Accounting accounting) throws URISyntaxException {
        ...
        if (accounting.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("accounting", "idexists", "A new accounting cannot already have an ID")).body(null);
        }
     ...
     * or with status 500 (Internal Server Error) if the accounting couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @RequestMapping(value = "/accountings",
    ...
    @RequestMapping(value = "/accountings",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Accounting>> getAllAccountings(Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to get a page of Accountings");
        Page<Accounting> page = accountingService.findAll(pageable); 
