...
/**
 * Performance test for the Taxpayer_client entity.
 */
class Taxpayer_clientGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Taxpayer_client entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all taxpayer_clients")
            .get("/api/taxpayer-clients")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created taxpayer_client")
                .get("${new_taxpayer_client_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created taxpayer_client")
            .delete("${new_taxpayer_client_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
