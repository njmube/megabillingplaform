...
/**
 * Performance test for the Free_concept entity.
 */
class Free_conceptGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Free_concept entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all free_concepts")
            .get("/api/free-concepts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created free_concept")
                .get("${new_free_concept_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created free_concept")
            .delete("${new_free_concept_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
